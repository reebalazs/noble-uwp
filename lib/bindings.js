...
NobleBindings.prototype._onAdvertisementWatcherReceived = function(sender, e) {
	...
	//
	// XXX workaround: never receive the manufacturerData without this workaround with Estimote beacons.
	// This seems to fix the adverticments at least, but it is only a temporary solution.
	if (true) {
	// if (isScanResponse) {
		let advertisement = {
